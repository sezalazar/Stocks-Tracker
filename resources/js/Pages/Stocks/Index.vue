...
const rsiChartData = computed(() => {
    const sorted = [...props.rsiData].sort((a, b) => new Date(a.data_timestamp).getTime() - new Date(b.data_timestamp).getTime())
...
const macdChartData = computed(() => {
    const sorted = [...props.macdData].sort((a, b) => new Date(a.data_timestamp).getTime() - new Date(b.data_timestamp).getTime())
    return sorted.map((item) => ({
        date: new Date(item.data_timestamp).toISOString().split('T')[0],
        macd: item.value,
        signal: item.signal,
...
const priceChangeClass = computed(() => {
    ...
    return priceChangeData.value.changePercent > 0 ? 'text-green-600' : priceChangeData.value.changePercent < 0 ? 'text-red-600' : ''
})
            ...
            <div class="p-6 space-y-8 bg-linear-to-r from-[hsl(var(--primary)/0.1)] via-white to-gray-100 rounded-xl shadow-lg">
                <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                    <div class="flex items-center gap-6">
                        <div
                            v-if="companyData.results?.logo_url"
                            class="w-20 h-20 shrink-0 bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200"
                        >
                        ...
                        <div>
                            ...
                            <div v-else class="text-sm text-gray-500 mt-2">No data available for price change.</div>
                        </div>
                    </div>
